...
class SqliteDB {
    constructor() {
        ...
        this.dbPath = path_1.default.join(baseDir, 'realm-system.db');
    }
    static getInstance() {
    ...
    ensureFile() {
        ...
        if (!fs_1.default.existsSync(this.dbPath)) {
            fs_1.default.writeFileSync(this.dbPath, '');
        }
    }
    open() {
        if (this.db)
    ...
    encode(v) { return typeof v === 'object' && v !== null ? JSON.stringify(v) : v; }
    decode(row) {
    ...
    delete(table, id) { this.run(`DELETE FROM ${table} WHERE id = ?`, id); }
    updateFields(table, id, patch) {
