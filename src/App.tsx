    ...
    if (sessionId) {
      const verifySession = async () => {
        try {
          ...
          setMessage('There was an issue verifying your payment. Please check your subscription status or contact support.');
          setIsError(true);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center">
      <div className="text-center p-6">
        <h1 className={`text-2xl font-bold mb-4 ${isError ? 'text-red-600' : 'text-green-600'}`}>
          {isError ? 'Payment Verification Error' : 'Payment Status'}
        </h1>
        <p className="text-gray-600 dark:text-gray-400">{message}</p>
        {!isError && <div className="mt-4 animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600 mx-auto"></div>}
      </div>
    </div>
  );
};
      ...
      <ThemeProvider>
        <ToastProvider>
          <AuthProvider>
            <Layout>
              <Routes>
                <Route path="/" element={!isConfigured ? <Navigate to="/setup" replace /> : <Dashboard />} />
                <Route path="/sales" element={!isConfigured ? <Navigate to="/setup" replace /> : <Sales />} />
                <Route path="/sales/full" element={!isConfigured ? <Navigate to="/setup" replace /> : <FullSale />} />
                <Route path="/expenses" element={!isConfigured ? <Navigate to="/setup" replace /> : <Expenses />} />
                <Route path="/products" element={!isConfigured ? <Navigate to="/setup" replace /> : <Products />} />
                <Route path="/settings" element={!isConfigured ? <Navigate to="/setup" replace /> : <Settings />} />
                <Route path="/componentes" element={!isConfigured ? <Navigate to="/setup" replace /> : <ComponentsTest />} />
                <Route path="/persons" element={!isConfigured ? <Navigate to="/setup" replace /> : <PersonClientManager />} />
                <Route path="/login" element={<Login />} />
                <Route path="/income" element={!isConfigured ? <Navigate to="/setup" replace /> : <Income />} />
                <Route path="/setup" element={<SetupWizard />} />
                <Route path="/subscription" element={!isConfigured ? <Navigate to="/setup" replace /> : <SubscriptionStatus />} />
