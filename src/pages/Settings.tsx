...
const Settings = () => {
  ...
  const loadCurrentInterval = async () => {
    try {
      const { data, error } = await supabaseService.functions.invoke('check-premium-subscription');
      if (!error && data?.isPremium && (data.interval === 'month' || data.interval === 'year')) {
            ...
            {monthlyPlan && (
              <div className="border dark:border-gray-700 rounded-lg p-6 flex flex-col">
                ...
                <button
                  ...
                  disabled={isCreatingSubscription || (subscriptionStatus === 'active' && planName === 'premium' && currentInterval === 'month')}
